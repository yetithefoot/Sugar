  ...
  var HASH_METHODS                = ['watch', 'unwatch', 'isEqual', 'values', 'toQueryString', 'has', 'size', 'keys'];
  var WRAPPED_HASH_METHODS        = ['merge', 'tap', 'each'];
  var WRAPPED_RESULT_HASH_METHODS = ['clone', 'map', 'select', 'reject'];
  ...
  function iterateOverProperties(hidden, obj, fn) {
    if (PROPERTY_DESCRIPTOR_SUPPORT && hidden) {
  ...
  function defineWrappedHashMethod(name, method) {
    setProperty(Hash.prototype, name, function() {
      this.data = method.apply(this.data, arguments);
      return this;
    });
  }

  function defineWrappedResultHashMethod(name, method) {
    setProperty(Hash.prototype, name, function() {
      return new Hash(method.apply(this.data, arguments));
    });
  }

  function buildHashMethods() {
    HASH_METHODS.forEach(function(name) {
      defineHashMethod(name, sugarObject[name].instance);
    });
    WRAPPED_HASH_METHODS.forEach(function(name) {
      defineWrappedHashMethod(name, sugarObject[name].instance);
    });
    WRAPPED_RESULT_HASH_METHODS.forEach(function(name) {
      defineWrappedResultHashMethod(name, sugarObject[name].instance);
    });
    defineHashMethod('get', wrapInstanceMethodFixed(sugarObject.get));
    defineHashMethod('set', wrapInstanceMethodFixed(sugarObject.set));
  }
