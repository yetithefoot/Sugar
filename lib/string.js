...
// Regex matching String#format tokens.
...
var ermouthAssign = (function(){
  var repo = new Object(null);
  return function assign(str, args){

    var arg1 = args[0] && args[0].valueOf(), reg = /(\{*)([^{}]*)(\}*)/g;

    // Unwrap if a single object is passed in.
    if (args.length === 1 && isObjectType(arg1)) {
      args = arg1;
    }
    return stringFormatCompiler(str, args);



    var i=0, acc, a, pos=0, key="", h = "", v, row;
    if (!(acc = repo[str])) {
      a = str.split("{");
      ...
      for (;i<a.length;i++){
        ...
        if ((pos=row.indexOf("}"))>0) {
          ...
          acc.push(key, row.substr(pos+1));
        } 
        else acc.push(null, (i?"{":"")+row);
      }
    }
    for (i=0;i<acc.length;i+=2) {
      key = acc[i];
      if (key !== null) {
        v = deepGetProperty(args, key);
        if (null != v) h+=v;
        else h+="{"+key+"}";
      }
      h+=acc[i+1];
...
defineInstanceWithArguments(sugarString, {
  ...
  'format': function(str, args) {
    var arg1 = args[0] && args[0].valueOf(), reg = STRING_FORMAT_TOKEN_REG;
    // Unwrap if a single object is passed in.
    if (args.length === 1 && isObjectType(arg1)) {
      args = arg1;
    }
    return str.replace(reg, function(match, open, close, empty, key, closeKey) {
      if (empty) {
        // {}
        return empty;
      } else if (open) {
  ...
  'format2': function(str, args) {
    ...
    for (var i = 0; i < keys.length; i++) {
      str = str.replace(new RegExp('\\{'+RegExp.escape(keys[i])+'\\}','g'), deepGetProperty(args, keys[i]));
    }
    return str;
  },

  'format3': function(str, args) {
    ...
    var result = '';
    var buffer = '';
    for (var i = 0; chr = str.charAt(i); i++) {
      isOpen = chr === '{';
      isClose = chr === '}';
      if (isOpen && lastWasOpen) {
        buffer += chr;
        isKey = false;
        lastWasOpen = false;
        continue;
        ...
        buffer = '';
        isKey = true;
      } else if (isClose && !isKey && str.charAt(i + 1) === '}') {
        buffer += chr;
        i++;
        continue;
      } else if (isClose) {
        ...
        if (buffer) {
          result += deepGetProperty(args, buffer);
        }
        buffer = '';
        isKey = false;
      } else {
  ...
  'format4': function(str, args) {
    var arg1 = args[0] && args[0].valueOf(), reg = STRING_FORMAT_TOKEN_REG;
    // Unwrap if a single object is passed in.
    if (args.length === 1 && isObjectType(arg1)) {
      args = arg1;
    }
    var keys = Object.keys(args), i=0, acc=[],
      a = str.split("{"), pos=0, key="", v, r;
      if (a.length==1) return str;
      for (;i<a.length;i++){
        if (a[i].length) {
          if ((pos=a[i].indexOf("}"))>0) {
            key = a[i].substr(0,pos);
            v= deepGetProperty(args, key);
            if (null != v) acc.push(v+a[i].substr(pos+1));
            else acc.push("{",a[i])
          } else acc.push("{",a[i]);
        }
      }
  ...
  'format5': function(str, args) {
    var openIndex, closeIndex, startIndex = 0, result = '', key;
    var arg1 = args[0] && args[0].valueOf();
    // Unwrap if a single object is passed in.
    if (args.length === 1 && isObjectType(arg1)) {
      args = arg1;
    ...
    do {
      ...
      if (closeIndex > openIndex) {
        key = str.slice(openIndex + 1, closeIndex);
        result += str.slice(startIndex, openIndex) + deepGetProperty(args, key);
  ...
  'format6': function(str, args) {
    ...
    str = str.replace(reg, function(match, open, close, empty, key, closeKey) {
        if (empty) {
          // {}
          return empty;
        } else if (open) {
  ...
  'format7': function(str, args) {
    ...
    str = str.replace(/\{([^{}]+)\}/g, function(match, key, index) {
  ...
  'format8': function(str, args) {
    ...
    return str.replace(reg, function(match, open, key, close) {
      var canMatch = !!open && !!close && key;
      var oLen = open.length % 2;
      var cLen = close.length % 2;
      if (oLen > cLen) {
        throw new Error('Unmatched { in format string.');
      } else if (cLen > oLen) {
        throw new Error('Unmatched } in format string.');
      ...
      if (open && close && key) {
        open = open.slice(0, open.length / 2);
        close = close.slice(0, close.length / 2);
        //var STRING_FORMAT_TOKEN_REG = /(\{*)(.*)(\}*)/g;
        return open + deepGetProperty(args, key) + close;
      }
      return match;
    });
  },

  'format9': function(str, args) {
    var arg1 = args[0] && args[0].valueOf(), reg = /(\{*)([^{}]*)(\}*)/g;
...
function foobar(key, args) {
  return deepGetProperty(args, key);
}

var stringFormatCompiler = createFormatCompiler(TEST_FORMAT_REG, foobar);
